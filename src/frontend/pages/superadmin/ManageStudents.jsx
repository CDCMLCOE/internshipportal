...
import { SearchBar } from '../../../frontend/components';
...
const ManageStudents = () => {
  ...
  const fetchStudents = async () => {
    ...
    try {
      ...
      const formatted = (data || []).map(p => ({
        id: p.id, name: p.name, email: p.email, branch: p.branch,
        year: p.year || 'N/A', prn: p.prn_no, status: 'Active'
      }));
  ...
  const handleSaveEdit = async () => {
    try {
      const { error } = await supabase.from('profiles').update({
        ...
        branch: editFormData.branch, year: editFormData.year, prn_no: editFormData.prn
      }).eq('id', editFormData.id);
      if (error) throw error;
      fetchStudents();
      setIsEditModalOpen(false);
      setSelectedStudent(null);
    } catch (error) { console.error('Error saving student:', error); alert('Failed to update student record.'); }
  };

  const handleAddStudent = async (newData) => {
    try {
      const { data, error } = await supabase.auth.signUp({
        email: newData.email, password: 'Temp@123',
        options: { data: { name: newData.name, role: 'student' } }
      });
      if (error) throw error;
      if (data?.user) {
        const { error: profileError } = await supabase.from('profiles').insert([{
          id: data.user.id, name: newData.name, email: newData.email,
          branch: newData.branch, year: newData.year, prn_no: newData.prn, role: 'student'
        }]);
        if (profileError) throw profileError;
      }
      alert('Student account created! Temporary password: Temp@123\nPlease ask the student to change their password on first login.');
      setIsAddModalOpen(false);
      fetchStudents();
    } catch (error) { console.error('Error adding student:', error); alert('Failed to create student: ' + error.message); }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (error) throw error;
      fetchStudents();
    } catch (error) { console.error('Error deleting student:', error); alert('Failed to delete student.'); }
  };
  ...
  return (
    <div className="space-y-10 pb-12">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 border-b border-mistral-black/10 pb-8">
        <div>
          <h3 className="font-portal font-bold text-lg tracking-tight text-mistral-orange italic mb-2"># Student Data</h3>
      ...
      <SearchBar value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
        placeholder="Search by name, email or branch..." wrapperClassName="relative w-full md:w-96 group"
        inputClassName="block w-full pl-10 pr-3 py-3 bg-brand-ivory border border-mistral-black/10 text-xs font-bold uppercase tracking-widest focus:outline-none focus:border-mistral-orange transition-all placeholder:text-mistral-black/20" />
            ...
            <div className="flex items-center gap-6 w-full md:w-auto">
              ...
              <div className="flex flex-col min-w-0">
                ...
                <div className="flex items-center gap-2 text-xs font-medium text-mistral-black/40 mt-1">
                  ...
                  <span>{student.branch} &bull; {student.year} &bull; PRN: {student.prn}</span>
                </div>
              </div>
            </div>
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)}
        panelClassName="relative w-full max-w-4xl bg-brand-ivory shadow-2xl overflow-hidden max-h-[90vh] flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              ...
              <select value={editFormData.branch} onChange={(e) => setEditFormData({...editFormData, branch: e.target.value})}
            ...
            <div className="space-y-1.5">
              ...
              <select value={editFormData.year} onChange={(e) => setEditFormData({...editFormData, year: e.target.value})}
                className="w-full bg-white border border-mistral-black/10 px-4 py-3 font-sans text-sm focus:outline-none focus:border-mistral-orange">
            ...
            <div className="space-y-1.5">
              ...
              <input type="text" value={editFormData.prn} onChange={(e) => setEditFormData({...editFormData, prn: e.target.value})}
                className="w-full bg-white border border-mistral-black/10 px-4 py-3 font-sans text-sm focus:outline-none focus:border-mistral-orange transition-all" />
            </div>
          </div>
        </div>
        <div className="p-6 border-t border-mistral-black/10 bg-brand-ivory flex justify-end gap-3">
          <button onClick={() => setIsEditModalOpen(false)}
            className="px-6 py-3 text-xs font-bold uppercase tracking-widest text-mistral-black/50 hover:text-mistral-black transition-colors">Cancel</button>
      ...
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)}
        panelClassName="relative w-full max-w-2xl bg-brand-ivory shadow-2xl overflow-hidden"
        showCloseButton={false}>
        ...
        <form className="p-8 space-y-6" onSubmit={(e) => {
          ...
          handleAddStudent({
            ...
            branch: formData.get('branch'), year: formData.get('year'),
            prn: formData.get('prn'), role: 'Not Assigned',
          });
        }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1.5">
          ...
          <div className="pt-6 border-t border-mistral-black/5 flex justify-end gap-3">
            <button type="button" onClick={() => setIsAddModalOpen(false)}
              className="px-6 py-3 text-xs font-bold uppercase tracking-widest text-mistral-black/50 hover:text-mistral-black transition-colors">Cancel</button>
            <button type="submit"
              className="px-8 py-3 bg-mistral-black text-white text-xs font-bold uppercase tracking-widest hover:bg-mistral-orange transition-all shadow-lg">Create Account</button>
          </div>
        </form>
      </Modal>
    </div>
