...
const AdminInternships = () => {
  ...
  const handleApprove = async (id) => {
    const { error } = await supabase.from('internships').update({ approval_status: 'approved' }).eq('id', id);
    if (error) {
      if (import.meta.env.VITE_USE_MOCK_AUTH === 'true') { setInternships(prev => prev.map(job => job.id === id ? { ...job, approval_status: 'approved' } : job)); return; }
  ...
  const handleReject = async (id) => {
    const { error } = await supabase.from('internships').update({ approval_status: 'rejected' }).eq('id', id);
    if (error) {
      if (import.meta.env.VITE_USE_MOCK_AUTH === 'true') { setInternships(prev => prev.map(job => job.id === id ? { ...job, approval_status: 'rejected' } : job)); return; }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this internship?')) {
      ...
      if (error) {
        if (import.meta.env.VITE_USE_MOCK_AUTH === 'true') { setInternships(prev => prev.filter(job => job.id !== id)); return; }
        console.error('Error deleting internship:', error); alert(`Failed to delete internship: ${error.message}`);
  ...
  const openEdit = (job) => {
    setEditingJob(job);
    setEditForm({
      title: job.title, location: job.location, category: job.type, status: job.status,
  ...
  const closeEditModal = () => { setIsEditModalOpen(false); setEditingJob(null); setEditForm({}); };

  const handleUpdate = async () => {
    if (!editForm.title.trim() || !editForm.location.trim()) return;
    const { error } = await supabase.from('internships').update({
      title: editForm.title, location: editForm.location, type: editForm.category, status: editForm.status,
      stipend: editForm.stipend, duration: editForm.duration, deadline: editForm.deadline,
      about_company: editForm.about_company, role_description: editForm.role_description,
      requirements: editForm.requirements.split('\n').filter(r => r.trim()),
      skills: editForm.skills.split(',').map(s => s.trim()).filter(s => s),
      perks: editForm.perks.split(',').map(p => p.trim()).filter(p => p)
    }).eq('id', editingJob.id);
    if (error) { console.error('Error updating internship:', error); alert(`Failed to update: ${error.message}`); return; }
    fetchInternships();
    closeEditModal();
  };
    ...
    <div className="space-y-8 pb-12">
      {statusMessage && (
        <div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 text-xs font-bold uppercase tracking-wide flex items-center justify-between gap-4">
          ...
          <button type="button" onClick={() => setStatusMessage('')} className="text-amber-800/60 hover:text-amber-950">Dismiss</button>
        </div>
      )}

      <PageHeader title="Manage Internships" subtitle="Add, edit, or remove internship listings from the portal.">
        <SearchBar value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search internships or companies..." />

        <FilterDropdown
          isOpen={isApprovalFilterOpen}
          onToggle={() => setIsApprovalFilterOpen(!isApprovalFilterOpen)}
          onClose={() => setIsApprovalFilterOpen(false)}
          triggerLabel={<>Approval: <span className="text-mistral-orange">{approvalFilter}</span></>}
        >
          ...
          {['All','pending','approved','rejected'].map((a) => (
            <button key={a} onClick={() => { setApprovalFilter(a); setIsApprovalFilterOpen(false); }}
              ...
              }`}>{a === 'All' ? 'All' : a.charAt(0).toUpperCase() + a.slice(1)}</button>
          ))}
        </FilterDropdown>
        ...
        <FilterDropdown
          ...
          triggerLabel={<>Category: <span className="text-mistral-orange">{activeFilter}</span></>}
        >
          ...
          {statuses.map((status) => (
            <button key={status} onClick={() => { setActiveFilter(status); setIsFilterOpen(false); }}
              className={`w-full text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest transition-colors border-b border-mistral-black/5 last:border-0 ${
          ...
          <button onClick={() => { setActiveFilter('All'); setIsFilterOpen(false); }}
            className="w-full text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-mistral-orange hover:bg-mistral-orange hover:text-white transition-colors">Clear Filters</button>
        </FilterDropdown>
      </PageHeader>
          ...
          <tbody>
            {filteredInternships.length > 0 ? (
              filteredInternships.map((job) => (
                <motion.tr key={job.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="group hover:bg-mistral-orange/5 transition-colors">
                  ...
                  <td className="p-4 border-b border-mistral-black/5">
                    <StatusBadge status={job.category} baseClassName="text-[10px] uppercase font-bold px-2 py-1" />
                  </td>
                  <td className="p-4 border-b border-mistral-black/5">
      ...
      <div className="flex justify-center md:justify-end">
        <Link to="/admin/internship-access"
      ...
      {/* Edit Modal */}
      <Modal isOpen={isEditModalOpen} onClose={closeEditModal} maxWidth="max-w-xl"
        panelClassName="relative w-full bg-brand-ivory rounded-xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto"
